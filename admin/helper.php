...
<?php
	  ...
	  switch($_GET['type']) {
		  ...
		  case "couponspecialssearch" :
		  	...
		  	$sql = "
					...
					" . Content::sTable . " s
					ON cs.store_id = s.id
					WHERE MATCH (cs.coupon_title) AGAINST ('" . $string . "*' IN BOOLEAN MODE)
					  OR MATCH (cs.coupon_type) AGAINST ('" . $string . "*' IN BOOLEAN MODE)
					  OR MATCH (cs.storename_ws) AGAINST ('" . $string . "*' IN BOOLEAN MODE)
					ORDER BY cs.active DESC, cs.year DESC, cs.id DESC
					LIMIT 10;";
		  	...
		  	$html .= '<div class="wojo small feed results segment">';
		  	foreach ( $result as $row ) :
		  	$link = Url::adminUrl ( "couponspecials", "edit", false, "?id=" . $row->csid );
		  	$html .= '<div class="event">';
		  	$html .= '<div class="label">';
		  	$html .= '<img src="'. $row->storeLogo . '" alt="">';
		  	$html .= '</div>';
		  	$html .= '<div class="content">';
		  	$html .= '<div class="date">';
		  	$html .= Utility::dodate ( 'long_date', $row->created ) . '<br>';
		  	$html .= Utility::dodate ( 'long_date', $row->modified );
		  	$html .= '</div>';
		  	$html .= '<div class="summary">';
		  	$html .= '<a href="' . $link . '">' . $row->coupon_title . '</a> <small>(' . $row->year . ')</small>';
		  	$html .= '</div>';
		  	$html .= '<div class="extra text">';
		  	$html .= '<p>' . Lang::$word->WSP_PRICE . ': $ ' . $row->coupon_price_amount  . '.' . $row->coupon_cents  . '</p>';
		  	$html .= '</div>';
		  	$html .= '</div>';
		  	$html .= '</div>';
		  	endforeach;
  ...
  if (isset($_POST['quickedit'])):
      ...
      switch ($_POST['type']) {
          ...
          case "pricediscount":
          	...
          	if ($_POST['key'] == "name"):
          		$data = array('name' => Validator::sanitize($_POST['title'])
          				, 'modified' =>Db::toDate()
          				, 'modified_id' => App::get('Auth')->uid);
          	    //$idata['update_flag'] = 1;
          	elseif ($_POST['key'] == "price"):
          		$data = array('price' => Validator::sanitize($_POST['title'])
          				, 'modified' =>Db::toDate()
          				, 'modified_id' => App::get('Auth')->uid);
          		 //$idata['update_flag'] = 1;
          	elseif ($_POST['key'] == "ordering"):
          	$data = array('ordering' => Validator::sanitize($_POST['title'])
          			, 'modified' =>Db::toDate()
          			, 'modified_id' => App::get('Auth')->uid);
          	     //$idata['update_flag'] = 1;
          	endif;
          		$db->update(wSpecials::wspTable, $data, array('id' => Filter::$id));
          		//$db->update(wSpecials::w_sTable, $idata, array('webspecials_id' => $_POST['webspecials_id'] ));
          		//
          		break;
          	
          	/* == Update Webspecials Ordering == */
          	case "webspecialordering":
          		if (empty($_POST['title'])):
          		print '--- EMPTY STRING ---';
          		exit;
          		endif;
          		if ($_POST['key'] == "ordering"):
          		$data = array('ordering' => Validator::sanitize($_POST['title'])
          				, 'modified' =>Db::toDate()
          				, 'modified_id' => App::get('Auth')->uid);
          		$idata = array('ordering' => Validator::sanitize($_POST['title'])
          				, 'modified_ws' =>Db::toDate()
          				, 'modified_id' => App::get('Auth')->uid);
          		endif;
          		$db->update(wSpecials::wsTable, $data, array('id' => Filter::$id));
          		$db->update(wSpecials::w_sTable, $idata, array('webspecials_id' => Filter::$id));
          		break;
          ...
          case "year":
          	...
          	case "dealtype":
          		...
          		case "coupontype":
          			...
          			$db->update(Content::cptypeTable, $data, array('id' => Filter::$id));
          			$sdata['coupontype_list'] = serialize($content->getCoupontype());
          			$db->update(Core::sTable, $sdata, array("id" => 1));
          			break;
          ...
          case "addNewFuel":
			  ...
			  <div class="wojo small form">
				<form method="post" id="wojo_form" name="wojo_form">
              	...
              	/* == Add New Coupon == */
              	case "addNewCoupontype":
              		print '
			  <div class="wojo small form">
				<form method="post" id="wojo_form" name="wojo_form">
				  <div class="field">
					<label>' . Lang::$word->CPTYPE_NAME . '</label>
					<div class="wojo labeled icon input">
					  <input type="text" placeholder="' . Lang::$word->CPTYPE_NAME . '" name="name" required>
					  <div class="wojo corner label"> <i class="icon asterisk"></i> </div>
					</div>
				  </div>
				  <input name="action" type="hidden" value="processCoupontype">
				</form>
			  </div>';
              		break;
  ...
  $sql = "
  	...
  	print '<div style = "text-align: right;">
      	   <input type="button" value="Print Now" class="ncpsbtn '.$cpspecialsStylesbtn.'"/>
      	   </div>
           <br>
      	   <div class="ncpsrow" id="cpspecialsPrintOutput_'.$row->csid.'">
      	   <div class="qncpsPage">
      	   <div class="NCPSPOPUPINDV">
      	   <div  class="ncpspanel '.$cpspecialsStyles.' coupon">
      	   ...
      	   <span class="exp">Expires: '. $expireDate.'</span>                        
      	   <div class="ncps_coupon_info_wrap">'.$couponInfoArea.'</div>
      	   </div>
      	   <div class="ncps_cost_right">
      	   <div class="offer" style="display:'.$showhidePrice.'" >
      	   <span class="usd">$</span>
      	   <span class="number">'. $couponPriceAmount.'</span>
      	   <span class="cents"'.$priceCentsDivStyle.'>'. $couponCents.'</span>
      	   </div>
      	   </div>
      	   <div class="ncpsclearfix"></div>
      	   <div class="ncpsdiscwrap">
      	   <div class="displayImage">
      	   <div class="showHideimg" style="display:'.$showhideImage.'"><img src="'. $couponImage.'" class="coupon-img img-rounded" height="250"></div>
      	   </div>
      	   <p class="disclosure">'.$disclaimerText.'</p>
      	   </div>
      	   <div class="'.$cpspecialsStyles.' ncpswell">
      	   <div id="business-info">
      	   <div class="ncps_storename"><div class="ncps_store_name_wrap">'.$dealerName.'</div></div>
      	   <div class="ncps_store_phone"><div class="ncps_store_phone_wrap"> <i class="fa fa-phone"></i>'.$dealerPhone.'</div></div>
      	   <div class="ncps_store_address"><span><i class="fa fa-map-marker"></i> '.$dealerAddress.', '.$dealerCity.', '.$dealerState.'</span></div>                    
      	   </div>
      	   </div>
      	   </div>
      	   </div>
           ...
           <div style = "text-align: right;">
      	   <input type="button" value="Close" class="ncpsbtn '.$cpspecialsStylesbtn.'"/>
      	   </div>
  	       </div>';
  ...
  /* == Load Web Specials Preview == */
  if (isset($_GET['loadWebSpecialsPreview'])):
  $id = Validator::sanitize($_GET['value']);
  $sletter = Validator::sanitize($_GET['value2']);
    Content::getwebspecialspreview($id,$sletter);
  endif;
  
   
   /* == Load Coupon Specials Preview == */
   if (isset($_GET['loadCouponSpecialsPreview'])):
    $id = Validator::sanitize($_GET['id']);
    Content::getCouponspecialspreview($id);
   endif;
   
   /* == Load Coupon Specials Preview Print== */
   if (isset($_GET['loadCouponSpecialsPreviewPrint'])):
   $id = Validator::sanitize($_GET['id']);
   Content::getCouponspecialsPreviewPrint($id);
   endif;
   
   /* == Load Mutliple Coupon Specials Preview == */
   if (isset($_GET['loadMultipleCoupon'])):
   $id = Validator::sanitize($_GET ['listid']);
   Content::getMulticouponpreview($id);
   endif;
   
   /* == Load Selected Dealerships Preview == */
   if (isset($_GET['loadSelectedDealership'])):
   $id = Validator::sanitize($_GET ['listid']);
   Content::getMultiSelectDealershipsPreview($id);
   endif;
   
   /* == Load Service Coupon Preview == */
   if (isset($_GET['loadServiceCoupon'])):
   $id = Validator::sanitize($_GET ['listid']);
   Content::getServiceCouponInformation($id);
   endif;
   
   /* == Load Service Coupon Description Disclaimer Preview == */
   if (isset($_GET['loadServiceCouponDescrDisclaimer'])):
   $id = Validator::sanitize($_GET ['listid']);
   Content::getServiceCouponDescDisclaimer($id);
   endif;
   
   /* == Load Selected Dealerships ADDBTN Preview == */
   if (isset($_GET['loadSelectedDealershipADDBTN'])):
   $id = Validator::sanitize($_GET ['listid']);
   Content::getMultiSelectDealershipsAddBTN($id);
   endif;
  
  /* == Load Web Specials Preview by Dealership== */
  if (isset($_GET['loadWebSpecialsPreviewbyDealership'])):
  $storeid = Validator::sanitize($_GET['storeid']);
  $id = Validator::sanitize($_GET['id']);
  Content::getwebspecialspreviewbystore($storeid,$id);
  endif;
