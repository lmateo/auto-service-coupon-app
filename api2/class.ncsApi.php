...
class ncsApi {
	...
	public $makesTblName = "makes";
	public $modelsTblName = "models";
	...
	{	
		$this->apiSqlQuery = "SELECT *, 
							...
							web_special_id=".$this->specialsTblName.".id AND domain_name='".addslashes($this->apiDataArr['wp_sp_domain'])."' ) as post_id  
	...
	{		
		...
		if( $this->apiTotalSpecialsFound > 0 ) 
		{
			...
			while( $row = mysqli_fetch_array($this->apiSqlResult) )
			{
				...
				$this->retXml .= "<special id=\"" . stripslashes($row['id']) . "\" post_id=\"" . stripslashes($row['post_id']) . "\" store_letter=\"" . stripslashes($row['store_letter']) . "\" ordering=\"".stripslashes($row['ordering'])."\" leads=\"\">\r\n";
				...
				$sql2 = "SELECT * FROM ".$this->specialPricingTblName." WHERE special_id='".$row['id']."' AND active='1' ORDER BY ordering";
