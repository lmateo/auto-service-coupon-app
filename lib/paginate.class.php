<?php
  ...
  class Paginator
  {
      ...
      private function __construct()
      {
          $this->current_page = 1;
          $this->mid_range = 7;
          $this->items_per_page = (isset($_GET['ipp']) and !empty($_GET['ipp'])) ? Validator::sanitize($_GET['ipp'], "int") : $this->default_ipp;
      }
      ...
      public function items_per_page()
      {
          ...
          return ($this->num_pages >= 1) ? "<select data-cover=\"true\" onchange=\"window.location='" . SITEURL . "/" . App::get('Core')->_urlParts .
              "?pg=1&amp;ipp='+this[this.selectedIndex].value+'';return false\">$items</select>\n" : '';
      }
      
      /**
       * Paginator::items_per_pagebyid($id)
       *
       * @return
       */
      public function items_per_pagebyid($qstringtype,$id)
      {
      	$items = '';
      	$ipp_array = array(
      			5,
      	...
      	foreach ($ipp_array as $ipp_opt)
      		...
      		"<option value=\"$ipp_opt\">$ipp_opt</option>\n";
      		return ($this->num_pages >= 1) ? "<select data-cover=\"true\" onchange=\"window.location='" . SITEURL . "/" . App::get('Core')->_urlParts .
      		"?$qstringtype=$id&pg=1&amp;ipp='+this[this.selectedIndex].value+'';return false\">$items</select>\n" : '';
      }
      
      /**
       * Paginator::jump_menubyid($id)
       *
       * @return
       */
      public function jump_menubyid($qstringtype,$id)
      {
      	$option = '';
      	...
      	return ($this->num_pages >= 1) ? "<select data-cover=\"true\" onchange=\"window.location='" . SITEURL . "/" . App::get('Core')->_urlParts .
      	"?$qstringtype=$id&pg='+this[this.selectedIndex].value+'&amp;ipp=$this->items_per_page';return false\">$option</select>\n" : '';
      }
      ...
      public function jump_menu()
      {
          ...
          return ($this->num_pages >= 1) ? "<select data-cover=\"true\" onchange=\"window.location='" . SITEURL . "/" . App::get('Core')->_urlParts .
              "?pg='+this[this.selectedIndex].value+'&amp;ipp=$this->items_per_page';return false\">$option</select>\n" : '';
      }
