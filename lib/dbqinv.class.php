<?php
   ...
   * @copyright 2018
   * @version $Id: dbqinv.class.php, v1.00 2018-02-15 18:20:24 gewa Exp $
   */
  if (!defined("_WOJO"))
      die('Direct access to this location is not allowed.');


  class Dbq extends PDO
  {
      ...
      public function __construct($params = array())
      {
          if (!empty($params)) {
              ...
              try {
                  if (DBQ_DATABASE != '') {
                      parent::__construct(DBQ_DRIVER . ':host=' . DBQ_SERVER . ';dbname=' . DBQ_DATABASE, DBQ_USER, DBQ_PASS, array(PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES utf8'));
       ...
       * @return
       */
      public function getValueByValue($sTable = '', $aColumn, $name = '')
      {
      	if($row = $this->select($sTable, array($aColumn), array('name' => $name), 'LIMIT 1')->result()) {
      		return $row->$aColumn;
      	} else {
      		return false;
      	}
       ...
       * @return
       */
      public function getValueByStoreId($sTable = '', $aColumn, $store_id = '')
      {
      	if($row = $this->select($sTable, array($aColumn), array('store_id' => $store_id), 'LIMIT 1')->result()) {
      		return $row->$aColumn;
      	} else {
      		return false;
      	}
