 ...
 */
  ...
  class wSpecials
  {
     ...
     /**
      ...
      public function processWebspecialsDubs()
      {
      	...
      	if (empty(Message::$msgs)) {
      		...
      		$data = array(
      		    ...
      		    $data['copy_link'] =  $data['store_letter']."-". $wbsp->id;
      			$data['created_ws'] = Db::toDate();
      			$data['created_id'] = App::get('Auth')->uid;
      			$data['idx'] = Utility::randNumbers();
              ...
              self::wscopylinkwspbyID($wsid);
      			 
      		   self::wspricingdbbyID($last_id);
       ...
       * @return
       */
      public static function copybulkSpecialsWebspecials($stletter, $storeletterS, $newlocation)
      {
      
      	
      	$store_letter_s = $stletter;
        $cwbsp = self::$db->select(self::wsTable, "*", array('store_letter' => $store_letter_s))->results();
      	$websptable = self::wsTable;
      	$storeletter = $storeletterS;
      	$location = $newlocation;
      
      	 
      	if($cwbsp){
      		foreach($cwbsp as $wbsp){
      			
      			$copyId = $wbsp->store_letter ."-". $wbsp->id;
      			$wsid = $wbsp->id;
      		...
      		self::wscopylinkwsp($stletter);
      		 
      		self::wspricingdb($storeletter);
      ...
      public function updatewspricing()
      {
      	...
      	if($cwbspricing){
      		foreach($cwbspricing as $wspricing){
      			    ...
      			    $websptable = self::wspTable3;
      			    
      			    $data = array('store_letter_ws' => $wspricingletter, 'stock_id_ws' => $wspricingstockid);
      			    
      				
      			self::$db->update(self::wspTable3, $data, array('special_id' => $wspricingid));
      ...
      /**
       * wSpecials::wscopylinkwsp()
       *
       * @return
       */
      public static function wscopylinkwsp($stletter)
      {
      	
      	$store_letter_s = $stletter;
      	$cwbspricing = self::$db->select(self::wsTable, "*", array('store_letter' => $store_letter_s))->results();
      	
      	if($cwbspricing){
      	foreach($cwbspricing as $wspricing){
       
      	$copyId = $wspricing->store_letter ."-". $wspricing->id;
      	$wsid = $wspricing->id;	
      		
        $pdata = array('copy_link_ws'=>$copyId);
        
       self::$db->update(self::wspTable, $pdata, array('special_id' =>$wsid));
      	
     
      }
      
        echo 'You have Updating Web Specials Pricing Copy Link';
      }else {
      	echo 'Error Updating Web Specials Pricing Copy Link';
      }
       ...
       * @return
       */
      public static function wspricingdb($store_letter)
      {
      
      	
      	$wstblsql = "
		  SELECT DISTINCT ws.id as id, ws.stock_id as stock_id, ws.store_letter, wsp.special_id, wsp.store_letter_ws, wsp.name, wsp.ordering, wsp.price, wsp.active
		  FROM
          	`" . self::wsTable . "` AS ws
          	RIGHT JOIN `" . self::wspTable . "` AS wsp
                	ON wsp.copy_link_ws = ws.copy_link
                		
                	WHERE ws.store_letter = '$store_letter'";
      ...
      /**
       * wSpecials::wscopylinkwspbyID()
       *
       * @return
       */
      public static function wscopylinkwspbyID($id)
      {
      	 
      	
      	$wspricing = self::$db->select(self::wsTable, "*", array('id' => $id))->result();
      ...
      /**
       * wSpecials::wspricingdbID()
       *
       * @return
       */
      public static function wspricingdbbyID($id)
      {
      	...
      	$wspricings = self::$db->pdoQuery($wstblsql)->results();
      	...
      	if($wspricings){
      			...
      			//$wspricingid = $wspricing->id;
      			//$wspricingletter = $wspricing->store_letter;
                foreach ($wspricings AS $wspricing) {
      			$wsdata = array(
      
      					'special_id'  => $wspricing->id,
      					'store_letter_ws' => $wspricing->store_letter,
      					'stock_id_ws' => $wspricing->stock_id,
      					'name' => $wspricing->name,
      					'price' => $wspricing->price,
      					'ordering' => $wspricing->ordering,
