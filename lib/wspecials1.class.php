     ...
     /**
      ...
      public function processWebspecials()
      {
          ...
          $validate->addSource($_POST);
           $validate->addRule('location','numeric', true, 1, 11, Lang::$word->WSP_ROOM);
           $validate->addRule('specials_type','numeric', true, 1, 11, Lang::$word->WSP_SPECIALS_TYPE);
           ...
           //$validate->addRule('title2','string', false);
           ...
           $validate->addRule('price_with_lease_conquest','string', false);
           $validate->addRule('price_with_owner_loyalty','string', false);
           $validate->addRule('custom_price_label','string', false);
           $validate->addRule('custom_price_val','string', false);
           $validate->addRule('vehicle_image','string', false); 
           $validate->addRule('mpg','string', false);
           $validate->addRule('alt_link_url','string', false);
           $validate->addRule('sale_end_date','string', false);
           $validate->addRule('sale_end_date_submit','string', false);
           $validate->addRule('brand_logo_url','string', false);
           $validate->addRule('new_fader_image_url','string', false);
           $validate->addRule('landing_page_url','string', false);
           $validate->addRule('vdp_url','string', false);
           $validate->addRule('youtube_video_id','string', false);
           $validate->addRule('ordering','numeric', false);
           $validate->addRule('lease_special_ordering','numeric', false);
           $validate->addRule('zero_down_lease_special_ordering','numeric', false);  
          // $validate->addRule('featured_special','numeric', false);
          ...
          if (empty(Message::$msgs)) {
              ...
              $data = array(
              		...
              		'specials_type' => $validate->safe->specials_type,
              		...
              		'lease_extras' => $validate->safe->lease_extras,
              		'lease_price' => $validate->safe->lease_price,
              		'lease_term' => $validate->safe->lease_term,
              		'single_zero_lease' => $validate->safe->single_zero_lease,
              		'single_lease_price' => $validate->safe->single_lease_price,
              		...
              		'price_with_lease_conquest' => $validate->safe->price_with_lease_conquest,
              		'price_with_owner_loyalty' => $validate->safe->price_with_owner_loyalty,
              		'custom_price_label' => $validate->safe->custom_price_label,
              		'custom_price_val' => $validate->safe->custom_price_val,
              		'vehicle_image' => $validate->safe->vehicle_image,
              		'mpg' => $validate->safe->mpg, 
              		'alt_link_url' => $validate->safe->alt_link_url,
              		'sale_end_date' => $validate->safe->sale_end_date_submit,
              		'disclaimer_text' => $_POST['disclaimer_text'],
              		'zero_down_lease_disclaimer_text' => $_POST['zero_down_lease_disclaimer_text'],
              		'brand_logo_url' => $validate->safe->brand_logo_url,
              		'new_fader_image_url' => $validate->safe->new_fader_image_url,
              		'landing_page_url' => $validate->safe->landing_page_url,
              		'vdp_url' => $validate->safe->vdp_url,
              		'youtube_video_id' => $validate->safe->youtube_video_id,
              		'active' => isset($_POST['active']) ? $_POST['active'] : 0,
              		'ordering' => $validate->safe->ordering,
              		'lease_special_active' => isset($_POST['lease_special_active']) ? $_POST['lease_special_active'] : 0,
              		'lease_special_ordering' => $validate->safe->lease_special_ordering,
              		'zero_down_lease_special_active' => isset($_POST['zero_down_lease_special_active']) ? $_POST['zero_down_lease_special_active'] : 0,
              		'zero_down_lease_special_ordering' => $validate->safe->zero_down_lease_special_ordering ,
              		'featured_special' => isset($_POST['featured_special']) ? $_POST['featured_special'] : 0
              		);
      ...
      public function processWebspecialsDubs()
      {
      	...
      	if (empty(Message::$msgs)) {
      		...
      		$data = array(
      				...
      				'price_with_lease_conquest' => $wbsp->price_with_lease_conquest,
      				'price_with_owner_loyalty' => $wbsp->price_with_owner_loyalty,
      				'custom_price_label' => $wbsp->custom_price_label,
      				'custom_price_val' => $wbsp->custom_price_val,
      				'vehicle_image' => $wbsp->vehicle_image,
      				'mpg' => $wbsp->mpg,
      				'alt_link_url' => $wbsp->alt_link_url,
      				'sale_end_date' => $wbsp->sale_end_date,
      				'disclaimer_text' => $wbsp->disclaimer_text,
      				'zero_down_lease_disclaimer_text' => $wbsp->zero_down_lease_disclaimer_text,
      				'brand_logo_url' => $wbsp->brand_logo_url,
      				'new_fader_image_url' => $wbsp->new_fader_image_url,
      				'landing_page_url' => $wbsp->landing_page_url,
      				'vdp_url' => $wbsp->vdp_url,
      				'youtube_video_id' => $wbsp->youtube_video_id,
      				'active' => $wbsp->active,
      				'ordering' => $wbsp->ordering,
      				'lease_special_active' => $wbsp->lease_special_active,
      				'lease_special_ordering' => $wbsp->lease_special_ordering,
      				'zero_down_lease_special_active' => $wbsp->zero_down_lease_special_active,
      				'zero_down_lease_special_ordering' => $wbsp->zero_down_lease_special_ordering,
      				'featured_special' => $wbsp->featured_special
      		);
      ...
      public function getPriceDiscounts()
      {
      	...
      	if (Filter::$id) {
      		$counter = self::$db->count(false, false, "SELECT COUNT(*) FROM `" . self::wspTable . "` WHERE special_id = " . Filter::$id . " LIMIT 1");
      	...
      	if (empty(Message::$msgs)) {
      		...
      		foreach ($_POST['name'] as $key => $val) {
					    ...
					    <td><small> ' . $last_id . '.</small></td>		
      		            <td data-editable="true" data-set=\'{"type": "pricediscount", "id": ' . $last_id . ' ,"key":"name", "path":""}\'>' . $npd->name . '</td>
      		            <td data-editable="true" data-set=\'{"type": "pricediscount", "id": ' . $last_id . ' ,"key":"price", "path":""}\'>' . $npd->price . '</td>
      		            <td data-editable="true" data-set=\'{"type": "pricediscount", "id": ' . $last_id . ' ,"key":"ordering", "path":""}\'>' . $npd->ordering . '</td>
